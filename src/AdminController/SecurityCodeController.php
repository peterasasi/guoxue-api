...
class SecurityCodeController extends BaseSymfonyApiController
{
    ...
    public function __construct(KernelInterface $kernel, ConfigServiceInterface $configService, SecurityCodeService $securityCodeService)
    {
     ...
     * @throws \Doctrine\ORM\OptimisticLockException
     */
    public function create($accepter, $codeType = '', $codeLength = 4, $codeRandType = StringHelper::ALPHABET)
    {
        ...
        $entity->setIp(ip2long($this->request->getClientIp()));
    ...
    public function createAndSend($accepter, $codeType, ValidatorInterface $validator, $codeLength = 6, $countryNo = '')
    {
        ...
        if ($this->ifEmail($accepter, $validator)) {
            return "email had sent. please check your email account after some seconds";
            ...
            $value = ConfigParser::parse($configEntity->getType(), $configEntity->getValue());
            if (is_array($value)) {
