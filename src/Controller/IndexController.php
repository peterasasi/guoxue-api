...
class IndexController extends BaseSymfonyApiController
{
    ...
    /**
     * @Route("/base", name="index_api_entry", methods={"POST","GET"})
     * @return Response
    ...
    public function index()
    {
        ...
        $this->logger->debug('auth check passed');
        ...
        $resp = $this->forward('App\\Controller\\' . $serviceName . 'Controller::' . $actionName, $params);
    ...
    public function checkAuthority($serviceType)
    {
        ...
        if ($cacheItem->isHit()) {
            ...
            $cacheItem->expiresAfter(ByCacheKeys::getExpireTime(ByCacheKeys::ApiIndexAuthPolices));
    ...
    /**
     * 表示SecurityCode_query服务结果缓存500秒
     * by_SecurityCode_query:500
     * @param $cacheKey
     * @return array|bool
     * @throws \Psr\Cache\InvalidArgumentException
     */
    protected function getCacheContent($cacheKey)
    {
        ...
        if ($this->cache->hasItem($cacheKey)) {
            ...
            $costTime = intval((microtime(true) - $startTime) * 1000000) . 'ms';
