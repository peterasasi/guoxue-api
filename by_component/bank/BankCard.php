...
class BankCard
{
    ...
    public static function info($cardNum)
    {
        ...
        if (empty($result)) {
            return CallResultHelper::fail('请求第三方服务无响应');
        }
        $result = json_decode($result);

        if (!$result->validated) {
            return CallResultHelper::fail('无法识别的银行卡号');
        } else {
            $cardTypeName = array_key_exists($result->cardType, self::$cardType) ? self::$cardType[strval($result->cardType)] : '';
            $bankInfo = array(
        ...
        return CallResultHelper::success($bankInfo);
    }
